      ...
      function (project, version, callback) {
        ...
        IterationModel
          ...
          .spread(function (iteration, created) {
            if (created) {
              console.log('迭代 - 项目[' + project.name + ']，版本[' + version.name + ']，迭代[' + iteration.name + ']导入成功，ID=' + iteration.id);
            } else {
              console.log('迭代 - 项目[' + project.name + ']，版本[' + version.name + ']，已存在迭代[' + iteration.name + ']，ID=' + iteration.id + '，忽略');
            }
            callback(null);
          });
      }
